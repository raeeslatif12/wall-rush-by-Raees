...
export async function handleAdminApi(request: Request): Promise<Response | null> {
  ...
  try {
    ...
    if (userMatch && request.method === "PATCH") { const id = userMatch[1]; const input = await body(request); const sets: string[] = []; const values: any[] = []; if (typeof input?.username === "string" && input.username.trim()) { sets.push("username"); values.push(input.username.trim()); } const numeric = ["points", "wins", "losses"] as const; for (const field of numeric) if (Number.isInteger(input?.[field])) { sets.push(field); values.push(input[field]); } if (input?.walls_per_player === null || Number.isInteger(input?.walls_per_player) && input.walls_per_player >= 1 && input.walls_per_player <= 50) { sets.push("walls_per_player"); values.push(input.walls_per_player); } if (typeof input?.disabled === "boolean") { sets.push("disabled"); values.push(input.disabled); } if (typeof input?.password === "string" && input.password.length >= 8) { sets.push("password_hash"); values.push(await hash(input.password, 12)); } if (!sets.length) return fail("No valid changes supplied."); const rows = await sql`SELECT id FROM users WHERE id=${id}`; if (!rows[0]) return fail("User not found.", 404); for (let i = 0; i < sets.length; i++) { const field = sets[i]; const value = values[i]; if (field === "username") await sql`UPDATE users SET username=${value} WHERE id=${id}`; if (field === "points") await sql`UPDATE users SET points=${value} WHERE id=${id}`; if (field === "wins") await sql`UPDATE users SET wins=${value} WHERE id=${id}`; if (field === "losses") await sql`UPDATE users SET losses=${value} WHERE id=${id}`; if (field === "walls_per_player") await sql`UPDATE users SET walls_per_player=${value} WHERE id=${id}`; if (field === "disabled") await sql`UPDATE users SET disabled=${value} WHERE id=${id}`; if (field === "password_hash") await sql`UPDATE users SET password_hash=${value} WHERE id=${id}`; } await audit("user_updated", actor.username, id, { fields: sets }); return json({ ok: true }); }

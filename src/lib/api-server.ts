...
type User = { id: string; email: string; username: string; points: number; games: number; wins: number; losses: number; streak: number; last_played: string | null; walls_per_player?: number | null; disabled?: boolean; last_active_at?: string | null };
...
const BOT_NAMES = ["Victor", "Alex", "Daniel", "Ryan", "Leo", "Max", "Oliver", "Ethan", "Noah", "Lucas", "Liam", "Jack"];
const DEFAULT_WALLS_PER_PLAYER = 12;
function botName(code: string) { return BOT_NAMES[[...code].reduce((sum, char) => sum + char.charCodeAt(0), 0) % BOT_NAMES.length]!; }
function wallInventory(count: number) { return { h: Math.ceil(count / 2), v: Math.floor(count / 2) }; }
...
async function sessionUser(request: Request): Promise<User | null> {
  ...
  try { const { payload } = await jwtVerify(token, secret); const rows = await sql`UPDATE users SET last_active_at=now() WHERE id=${String(payload.sub)} AND disabled=false RETURNING id,email,username,points,games,wins,losses,streak,last_played,walls_per_player,disabled,last_active_at`; return rows[0] as User ?? null; } catch { return null; }
}
...
async function ensureSchema() {
  ...
  schemaPromise ??= (async () => {
    ...
    await sql`INSERT INTO game_settings (key,value) VALUES ('walls_per_player',${DEFAULT_WALLS_PER_PLAYER}) ON CONFLICT (key) DO UPDATE SET value=CASE WHEN game_settings.value=10 THEN EXCLUDED.value ELSE game_settings.value END`;
  ...
  try {
    ...
    if (legacyRoomMatch && request.method === "PATCH") {
      ...
      if (input?.action === "join") {
        ...
        const settingRows = await sql`SELECT value FROM game_settings WHERE key='walls_per_player'`;
        const defaultCount = Number(settingRows[0]?.["value"] ?? DEFAULT_WALLS_PER_PLAYER);
        const joiningUser = await sessionUser(request);
        const count = joiningUser?.walls_per_player ?? defaultCount;
        const currentState = current.state as RoomState;
        const limits = currentState.wallLimits ?? [defaultCount, defaultCount];
        const inventory = { h: Math.ceil(count / 2), v: Math.floor(count / 2) };
        const nextState = { ...currentState, wallLimits: [limits[0], count] as [number | null, number | null], game: { ...currentState.game, wallsLeft: [currentState.game.wallsLeft[0]!, inventory] } };
        const updated = await sql`UPDATE rooms SET p2_token=${token},p2_name=${name},is_bot=false,status='playing',state=${JSON.stringify(nextState)}::jsonb,updated_at=now() WHERE code=${legacyRoomMatch[1]} AND p2_token IS NULL RETURNING *`;
    ...
    if (actionMatch && request.method === "PATCH") {
      ...
      if (action.type === "move" && action.to && Number.isInteger(action.to.r) && Number.isInteger(action.to.c)) {
        ...
        const defaultCount = Number(settingRows[0]?.["value"] ?? DEFAULT_WALLS_PER_PLAYER);
        const limits = currentState.wallLimits ?? [defaultCount, defaultCount];
        const inventory = (count: number | null) => ({ h: Math.ceil((count ?? defaultCount) / 2), v: Math.floor((count ?? defaultCount) / 2) });
        nextState = { ...currentState, game: { pawns: [{ r: 8, c: 4 }, { r: 0, c: 4 }], walls: [], wallsLeft: [inventory(limits[0]), inventory(limits[1])], turn: 0, winner: null, moveCount: 0, history: [] }, clocks: { base: [300, 300], lastMoveAt: Date.now() }, resignedBy: null, rematch: [false, false] };
        nextStatus = "playing";
        nextWinner = null;

  ...
  try {
    ...
    if (actionMatch && request.method === "PATCH") {
      ...
      if (action.type === "move" && action.to && Number.isInteger(action.to.r) && Number.isInteger(action.to.c)) {
        ...
        const count = Number(settingRows[0]?.["value"] ?? DEFAULT_WALLS_PER_PLAYER);
        const inventory = { h: Math.ceil(count / 2), v: Math.floor(count / 2) };
        nextState = { ...currentState, game: { pawns: [{ r: 8, c: 4 }, { r: 0, c: 4 }], walls: [], wallsLeft: [{ ...inventory }, { ...inventory }], turn: 0, winner: null, moveCount: 0, history: [] }, clocks: { base: [300, 300], lastMoveAt: Date.now() }, resignedBy: null, rematch: [false, false] };
        nextStatus = "playing";
        nextWinner = null;
    ...
    if (path === "/api/rooms" && request.method === "POST") { const input = await body(request); const token = text(input?.token, 100); const name = text(input?.name, 80); if (!token || !name) return error("Player identity is required."); const isPublic = input?.isPublic === true; const code = makeCode(); const settingRows = await sql`SELECT value FROM game_settings WHERE key='walls_per_player'`; const count = Number(settingRows[0]?.["value"] ?? DEFAULT_WALLS_PER_PLAYER); const inventory = { h: Math.ceil(count / 2), v: Math.floor(count / 2) }; const state = input?.state ?? { game: { pawns: [{ r: 8, c: 4 }, { r: 0, c: 4 }], walls: [], wallsLeft: [{ ...inventory }, { ...inventory }], turn: 0, winner: null, history: [], moveCount: 0 }, clocks: { base: [300, 300], lastMoveAt: Date.now() }, rematch: [false, false], resignedBy: null }; state.game.wallsLeft = [{ ...inventory }, { ...inventory }]; const rows = await sql`INSERT INTO rooms (code,is_public,status,p1_token,p1_name,state) VALUES (${code},${isPublic},'waiting',${token},${name},${JSON.stringify(state)}::jsonb) RETURNING *`; const created = rows[0]; return created ? response({ room: room(created) }, 201) : error("Could not create room.", 500); }
